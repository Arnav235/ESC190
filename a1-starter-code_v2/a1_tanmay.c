...
Order* build_order(char* items, char* quantities){
	...
	char array_quantities[strlen(quantities)+1];
	strcpy(array_quantities,quantities);
	char* token = strtok(array_quantities,MENU_DELIM);
	int index = 0;
	for(int i = 0; i < order->num_items; i++){
		order->item_quantities[i] = strtol(token, NULL, 10);
		order->item_codes[i] = (char*)malloc(sizeof(char)*ITEM_CODE_LENGTH);
		strncpy(order->item_codes[i], &items[index], ITEM_CODE_LENGTH-1);
		order->item_codes[i][ITEM_CODE_LENGTH-1] = '\0';
		token = strtok(NULL, MENU_DELIM);
		index += ITEM_CODE_LENGTH-1;
	}
	return order;
}
